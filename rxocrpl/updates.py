...
rec = lambda x: re.compile(r'.*' + re.escape(x) + r'.*', re.IGNORECASE)
...
def update_product_labelers():
      ...
      """
      products = list(
            Product.objects.filter(labeler__isnull=True)
            .only('pk', 'labeler_name', 'product_ndc')
      )
      if not products:
            return
      ...
      existing = {l.verbose_name: l for l in Labeler.objects.filter(verbose_name__in=unique_names)}
      ...
      if missing_names:
            sample_for_name = {}
            for p in products:
                  if p.labeler_name in missing_names and p.labeler_name not in sample_for_name:
            ...
            new_labelers = [
                  Labeler(
                        ...
                        labeler_code=(
                              ...
                              if sample.product_ndc else None
                        ),
                  )
      ...
      for product in products:
            product.labeler = existing[product.labeler_name] if product.labeler_name in existing else None
...
def _deactivate_by_product_filter(match_q):
      ...
      products_to_deactivate = Product.objects.filter(active=True).filter(match_q).distinct()
      product_ids = list(products_to_deactivate.values_list('pk', flat=True))

      # 2. Deactivate corresponding packaged products
      pkg_to_deactivate = PackagedProduct.objects.filter(active=True, product_id__in=product_ids)
...
def deactivate_hospital_irrelevant_labelers():
      cosmetic_patterns = [
            ...
            r'.*Avobenzone.*', r'.*Octisalate.*', r'.*Octocrylene.*', r'.*Oxybenzone.*',
            r'.*Zinc Oxide.*', r'.*Titanium Dioxide.*', r'.*Homosalate.*', r'.*Octinoxate.*',
            r'.*Ensulizole.*', r'.*Padimate.*', r'.*Sulisobenzone.*', r'.*Meradimate.*',
            r'.*Trolamine Salicylate.*', r'.*Sunscreen.*', r'.*SPF.*',
            # Hair Regrowth
            r'.*Minoxidil.*', '.*Hair Oil.*',
            # Antiseptics / hand sanitizer
            r'.*Benzethonium Chloride.*', r'.*Benzalkonium Chloride.*', r'.*Cetylpyridinium Chloride.*',
            r'.*Chlorhexidine.*', r'.*Triclosan.*', r'.*Hexylresorcinol.*',
            r'.*Hand Sanitizer.*', r'.*Alcohol.*', r'.*Isopropyl.*', r'.*Povidone.Iodine.*',
            # Acne / skin actives
            r'.*Salicylic Acid.*', r'.*Benzoyl Peroxide.*', r'.*Adapalene.*', ".*Hyaluronic Acid.*",
            r'.*Glycolic Acid.*', r'.*Retinol.*', r'.*Hydroquinone.*', r'.*Niacinamide.*',
            # Topical anesthetics
            r'.*\w+caine.*', r'.*Pramoxine.*', r'.*Phenol.*',
            # Skin protectants / emollients / diaper & anti-itch
            r'.*Petrolatum.*', r'.*Dimethicone.*', r'.*Glycerin.*', r'.*Lanolin.*',
            r'.*Allantoin.*', r'.*Calamine.*', r'.*Colloidal Oatmeal.*', r'.*Cocoa Butter.*',
            r'.*Shea Butter.*', r'.*Aloe.*', r'.*Urea.*',
            # Anti-itch / topical steroid & antihistamine
            r'.*Hydrocortisone.*', r'.*Diphenhydramine.*',
            # Antifungals (OTC topical)
            r'.*Clotrimazole.*', r'.*Miconazole.*', r'.*Terbinafine.*', r'.*Tolnaftate.*',
            r'.*Undecylenic Acid.*', r'.*Ketoconazole.*', r'.*Butenafine.*', r'.*Ciclopirox.*',
            r'.*Pyrithione.*',
            # Topical analgesics / counterirritants (rubs, patches)
            r'.*Menthol.*', r'.*Methyl Salicylate.*', r'.*Camphor.*', r'.*Capsaicin.*',
            r'.*Trolamine.*',
            # Oral care
            r'.*Fluoride.*', r'.*Sodium Monofluorophosphate.*', r'.*Stannous Fluoride.*',
            r'.*Potassium Nitrate.*', r'.*Carbamide Peroxide.*',
            # Eye / nasal / lip OTC
            r'.*Tetrahydrozoline.*', r'.*Naphazoline.*', r'.*Ketotifen.*',
            r'.*Oxymetazoline.*', r'.*Phenylephrine.*', r'.*Saline.*',
            r'.*Polyethylene Glycol.*', r'.*Carboxymethylcellulose.*', r'.*Hypromellose.*',
            # Wart / callus / astringent
            r'.*Witch Hazel.*',
            # Antiperspirant actives
            r'.*Aluminum Chlorohydrate.*', r'.*Aluminum Zirconium.*',
      ]
