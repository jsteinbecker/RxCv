...
class NDCEntry:
    ...
    def display_strength(self) -> Optional[str]:
        """Combine strength + unit into a human-readable string.

        For multi-component drugs all active substances are joined with ' / '
        so the result reflects the full label strength (e.g. '2 g / 0.5 g'
        for Avycaz) rather than only the primary substance."""
        if not self.strengths or not self.units:
            return None
        parts = []
        for s, u in zip(self.strengths, self.units):
            if not s or not u:
                continue
            # FDA units often look like 'g/1', 'mg/mL', 'meq/1'. Strip the
            # trivial '/1' denominator.
            u_clean = re.sub(r"/1\b", "", u)
            parts.append(f"{s} {u_clean}".strip())
        if not parts:
            return None
        return " / ".join(parts)
...
class NDCDirectory:
    ...
    def entries_for_generic(self, drug_name: str, max_results: int = 20) -> list[NDCEntry]:
        """Return up to max_results entries whose nonproprietary name contains drug_name."""
        if not self._loaded:
            self.load()
        query = drug_name.strip().upper()
        if not query or len(query) < 3:
            return []
        results: list[NDCEntry] = []
        for entry in self._by_ndc.values():
            if query in entry.nonproprietary_name.upper():
...
# ---------------------------------------------------------------------------
_DEFAULT_PATH = os.environ.get("NDC_DIRECTORY_CSV", "/Users/jts/PycharmProjects/RxCv/rxocrpl/db/ndcproduct.csv")
