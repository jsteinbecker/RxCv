...
class ClinicalDrug(ComputedFieldsModel):
      ...
      route = ComputedField(models.CharField(max_length=20, choices=RouteOfAdministration.choices),
                            compute=lambda self: self.concept.attributes.get('route', None),
                            depends=[('concept', ['attributes'])])
      quantified = ComputedField(models.BooleanField(default=False),
                                 depends=[('concept', ['name'])],
                                 compute=lambda self: check_quantified_form(self.concept.name))
...
class Product(ComputedFieldsModel):
      ...
      def from_fda_result(cls, result: object) -> "Product":
            ...
            for ai in active_ingredients:
                  if hasattr(ai, 'name'):
                        ingredients_data.append({'name': ai.name, 'strength': getattr(ai, 'strength', '')})
                  else:
      ...
      @classmethod
      def lookup_by_generic_name(cls,
                                 generic_name: str,
            ...
            results = lookup_generic_name(generic_name, dosage_form, brand_name=brand_name, fetch_rxcui=fetch_rxcui)
            if results:
      ...
      def as_substance(self):
            ...
            for ing in self.active_ingredients:
                  try:
                        ...
                        substances.append(SubstanceQuantity(
                              value=q.value,
                              unit=q.unit,
...
class PackagedProduct(ComputedFieldsModel):
      ...
      active = models.BooleanField(default=True)
      package_ndc = ComputedField(models.CharField(max_length=13, unique=True),
...
class VerificationImage(models.Model):
      ...
      def __str__(self):
            return f"Verification image for Order {self.order_id}"
...
class Facility(models.Model):
      ...
      name = models.CharField(max_length=255)
      organization = models.ForeignKey(Organization, on_delete=models.CASCADE, related_name='facilities', null=True,
                                       blank=True)
      ...
      def current_admins(self):
            ...
            ).exclude(role_grants__expires_at__lt=timezone.now()).distinct()
...
class User(models.Model):
      ...
      def has_role(self, role_name: str, facility: Facility = None, organization: Organization = None) -> bool:
            """
...
class RoleGrant(models.Model):
            ...
            constraints = [
                  models.CheckConstraint(
                        condition=(
                                  ...
                                  (models.Q(organization__isnull=True) & models.Q(facility__isnull=False))
                        ),
      ...
      def clean(self):
            ...
            if self.organization and self.facility:
                  raise ValidationError("A grant cannot have both an organization and a facility scope.")
            if not self.organization and not self.facility:
                  raise ValidationError("A grant must have either an organization or a facility scope.")
            ...
            if self.granted_by is None and self.reason != 'system_bootstrap':
                  raise ValidationError("granted_by can only be null if reason is 'system_bootstrap'.")
      ...
      def _granter_has_authority(self) -> bool:
            ...
            return (
                      self.granted_by.has_role('org_admin', organization=self.organization, facility=self.facility) or
                      self.granted_by.has_role('facility_admin', facility=self.facility)
            )
...
class RoleGrantEvent(models.Model):
      ...
      grant = models.ForeignKey(RoleGrant, on_delete=models.CASCADE, related_name='events')
      event_type = models.CharField(max_length=50)  # e.g., 'denied_attempt', 'auto_expiry', 'annotation'
      ...
      def __str__(self):
            return f"{self.event_type} on {self.grant_id} at {self.timestamp}"
...
class ApprovedProductReconstitutionScheme(models.Model):
      ...
      diluent_volume_ml = models.DecimalField(max_digits=12, decimal_places=4, null=True, blank=True)
      ...
      def whole_product_strength(self):
            return Quantity(self.whole_product_strength_mag, self.whole_product_strength_unit)
