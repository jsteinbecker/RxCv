...
class RxNormConcept(models.Model, RxConceptTraversalMixin):
    ...
    tty = models.CharField(
        max_length=10, choices=TermType.choices, null=True, blank=True
    )
...
class RxNormConceptRelation(models.Model):
    ...
    source = models.ForeignKey(
        RxNormConcept, related_name="outbound_relations", on_delete=models.CASCADE
    )
    target = models.ForeignKey(
        RxNormConcept, related_name="inbound_relations", on_delete=models.CASCADE
    )
    ...
    def __str__(self):
        return f"{self.source.id} -[{self.rela}]-> {self.target.id}"
...
class ClinicalDrug(ComputedFieldsModel):
    ...
    route = ComputedField(
        models.CharField(max_length=20, choices=RouteOfAdministration.choices),
        compute=lambda self: self.concept.attributes.get("route", None),
        depends=[("concept", ["attributes"])],
    )
    quantified = ComputedField(
