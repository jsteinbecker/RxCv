...
try:
    from .raw_input import RAW          # when imported as a package
...
LOT_DB = {
    "308777251000": {
        "drug": "(unidentified — lot 308777251000)",
        "strength": None,
        "manufacturer": None,
        "form": "vial",
    },
...
CLASS_PRIORITY = {
    "iv bag": 5,
    "vial": 4,
    "bottle": 2,   # often a misclassification of vial
...
# ---------------------------------------------------------------------------
def clean_detections(detections: list[dict], image_dims: dict,
                     iou_thresh: float = 0.7,
                     coverage_thresh: float = 0.80,
                     reflection_y_overlap: float = 0.05,
                     reflection_x_overlap: float = 0.30) -> list[dict]:
    ...
    for d in detections:
        ...
        if cov >= coverage_thresh:
            rejected_frame.append({**d, "_reject_reason": f"frame_coverage={cov:.2f}"})
        else:
            survivors.append(d)
        ...
        for d in dets:
            ...
            if is_reflection:
                rejected_reflection.append({
                    **d,
                    "_reject_reason": f"reflection_of_instance={parent}",
                })
            else:
                survivors2.append(d)
    ...
    for img, dets in by_image.items():
        ...
        dets_sorted = sorted(
            dets,
            key=lambda d: (CLASS_PRIORITY.get(d["class_label"], 0), d["score"]),
            reverse=True,
        )
        ...
        for d in dets_sorted:
            ...
            if matched_idx is None:
                ...
                absorbed_into.setdefault(k["instance_id"], []).append(d["instance_id"])
                merged_log.append({
                    "kept_instance": k["instance_id"],
                    "kept_class": k["class_label"],
                    "absorbed_instance": d["instance_id"],
                    "absorbed_class": d["class_label"],
...
def match(reference: list[dict], candidate: list[dict]) -> dict:
    ...
    for c in candidate:
        ...
        if key[0] in ("ndc", "drug", "lot") and ref_pool.get(key):
            ...
            assignments.append({
                "candidate_instance": c["instance_id"],
                "matched_reference_instance": slot["instance_id"],
                "match_key": key,
                "match_type": "hard",
                "score": 1.0,
    ...
    for c in deferred:
        ...
        if pool:
            ...
            assignments.append({
                ...
                "matched_reference_instance": slot["instance_id"],
                "match_key": ("class", cls),
                "match_type": "soft_class_only",
                "score": 0.4,
                "out_of_inventory": False,
                "needs_human_review": True,
                "review_reason": (
                    "Matched by class label only — OCR did not recover NDC/lot, "
            ...
            assignments.append({
                "candidate_instance": c["instance_id"],
                "matched_reference_instance": None,
                "match_key": product_key(c),
    ...
    unmatched_reference = [
        r["instance_id"]
        for slots in remaining_ref_by_class.values()
        for r in slots
    ]
...
# ---------------------------------------------------------------------------
def run():
    cleaned, rejected_frame, rejected_reflection, merged_log = clean_detections(
        RAW["detections"], RAW["image_dims"]
    )
    ...
    return {
        "rejected_frame_detections": [
            {"instance_id": r["instance_id"], "class_label": r["class_label"],
             "reason": r["_reject_reason"]}
            for r in rejected_frame
        ],
        "rejected_reflection_detections": [
            {"instance_id": r["instance_id"], "class_label": r["class_label"],
             "reason": r["_reject_reason"]}
            for r in rejected_reflection
        ],
        "merge_log": merged_log,
        "cleaned_detection_count_per_image": {
            img: sum(1 for d in resolved if d["source_image"] == img)
            for img in RAW["images"]
        },
        "rx3_inventory": rollup(ref),
        "rx4_inventory": rollup(cand),
        "matches": matches,
