...
LOT_RE = re.compile(
    r"(?:LOT|Lot|lot)\s*[:#]?\s*([A-Z0-9]{4,14})", re.IGNORECASE
)
...
class OCRFields:
    ...
    def needs_review(self, threshold: float = 0.4) -> bool:
        ...
        for value, conf in [
            (self.lot, self.lot_confidence),
            (self.exp, self.exp_confidence),
...
def _rotation_variants(crop: Image.Image) -> list[tuple[int, Image.Image]]:
    ...
    Knocked-over vials (image 4 in the reference set) need this. PaddleOCR
    has its own orientation classifier but it's unreliable on partial labels.
    """
...
class FieldExtractor:
    ...
    def __init__(self, lang: str = "en", use_gpu: bool = False) -> None:
        ...
        # use_angle_cls=True lets Paddle handle small rotations internally;
        # we still do explicit 90deg rotations because Paddle's classifier
        # is trained for ±15deg range, not full quarter turns.
    ...
    def extract(
        ...
        return OCRFields(
            lot=lot,
            exp=exp,
            ndc=_normalize_ndc(ndc) if ndc else None,
    ...
    def _ocr_one(self, img: Image.Image) -> list[str]:
        """Run PaddleOCR on a single PIL image, return list of detected strings."""
        arr = np.array(img)
        result = self.ocr.ocr(arr, cls=True)
        if not result or not result[0]:
            return []
        # PaddleOCR result format: [[ [box], (text, conf) ], ...]
