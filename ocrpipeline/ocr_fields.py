...
class FieldExtractor:
    ...
    def extract(
        ...
        lines, and the structured-field aggregator votes across pools."""
        if try_rotations:
    ...
    def _ocr_one(self, img: Image.Image) -> list[str]:
        """OCR a single PIL image; return detected text strings.

        EasyOCR returns (bbox, text, confidence) tuples. We discard bbox and
        per-detection confidence — the variant-ensemble vote is a more
        reliable signal than EasyOCR's overconfident per-box numbers."""
        arr = np.array(img)
